"""Install the 28 authored French workshops for human review. Never approves content."""
import argparse
import hashlib
import json
...
SOURCE_URL="https://www.education.gouv.fr/sites/default/files/programme-de-fran-ais-pour-le-cycle-3-439824.pdf"
...
def stage(engine,tenant,owner,source_file,apply=False):
    ...
    with tenant_session(engine,tenant) as session:
        ...
        comps={c.code:c for c in session.scalars(select(Competency).where(Competency.programme_version=="programme-2025-cm2-2026",Competency.code.in_([c['code'] for c in COURSES])))}
        assert len(comps)==len(COURSES)==28
        for c in COURSES:assert c['catalogue_objective'] in comps[c['code']].objectives
        if not apply:return {"courses":28,"exercises":84,"source_sha256":digest,"publication":"human_review_required"}
        ...
        if not source:
            source=ContentSource(tenant_id=tenant,title="Programme de français du cycle 3 — CM2, rentrée 2026",url=SOURCE_URL,publisher="Ministère de l’Éducation nationale",license="Référence officielle ; cours et exercices originaux BoostClasse.",checksum_sha256=digest,retrieved_at=datetime.now(timezone.utc))
            session.add(source);session.flush()
        created=0;result=[]
        for entry in COURSES:
            comp=comps[entry['code']];ids=[]
            for index,candidate in enumerate(bind_exercises(entry,comp.id,[source.id])):
                slug=entry['code'].lower()+f'-atelier-francais-{index}'
                ...
                if item:
                    ...
                    audit(session,operator,'operator.french_exercise_staged','exercise_version',version.id);created+=1
            ...
            if not lesson:
                ...
                version=LessonVersion(tenant_id=tenant,lesson_id=lesson.id,editor_id=None,version=1,title=entry['title'],body=payload['body'],status='draft');session.add(version);session.flush()
            ...
            result.append({'code':entry['code'],'lesson_id':str(lesson.id)})
