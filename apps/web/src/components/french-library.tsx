...
const domains=[
  {id:"grammaire",title:"Grammaire",verb:"Démonte les phrases",icon:Shapes,codes:["NATURES","FONCTIONS","OBJETS","ATTRIBUT","PHRASE","CIRCONSTANCES","PRONOMS","EXPANSIONS"],img:[418,310]},
  {id:"conjugaison",title:"Conjugaison",verb:"Fais voyager les verbes",icon:TimerReset,codes:["PRESENT","IMPARFAIT","FUTUR","PASSE-COMPOSE","PASSE-SIMPLE","PLUS-QUE-PARFAIT"],img:[384,329]},
  {id:"orthographe",title:"Orthographe",verb:"Trouve les bons accords",icon:SpellCheck,codes:["ACCORDS","HOMOPHONES","DICTEE-RELECTURE","PARTICIPE-ETRE","ON-ONT"],img:[385,311]},
  {id:"lexique",title:"Lexique",verb:"Joue avec le sens",icon:MessagesSquare,codes:["LEXIQUE","POLYSEMIE","SYNONYMES","ANTONYMES","DICTIONNAIRE"],img:[385,329]},
  {id:"lecture",title:"Lecture",verb:"Mène ton enquête",icon:BookOpen,codes:["COMPREHENSION","LECTURE-ENQUETE"],img:[385,292]},
  {id:"ecriture",title:"Écriture",verb:"Donne vie à tes idées",icon:PenLine,codes:["ECRIT-REDIGER","RESUMER"],img:[384,291]},
];
...
export function FrenchLibrary({topics,studentId,refreshKey=0,renderStart}:{topics:Competency[];studentId?:string;refreshKey?:number;renderStart:(topic:Competency)=>ReactNode}){
  ...
  return <div className="french-workshop">
    ...
    <section aria-labelledby="atelier-domaines"><div className="atelier-section-title"><div><p className="atelier-kicker">Choisis ton terrain de découverte</p><h2 id="atelier-domaines">La carte des territoires</h2></div><button className="text-link" aria-pressed={domain==="all"} onClick={()=>{setDomain("all");setQuery("");}}>Tout explorer</button></div>
      <div className="atelier-islands">{domains.map(d=>{const count=courseTopics.filter(t=>domainOf(t)?.id===d.id).length;return <button key={d.id} type="button" className={`island-tile ${domain===d.id?"active":""}`} aria-pressed={domain===d.id} aria-label={`${d.title} : ${d.verb}`} onClick={()=>setDomain(d.id)}><img src={`/carte-ile-${d.id}.webp`} alt="" width={d.img[0]} height={d.img[1]} draggable={false}/><span className="island-chip"><strong>{d.title}</strong><em>{d.verb}</em><small>{count} leçon{count>1?"s":""}</small></span></button>;})}</div>
    </section>
